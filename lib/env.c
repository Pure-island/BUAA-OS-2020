...
env_alloc(struct Env **new, u_int parent_id)
{
    int r;
    struct Env *e;
	//printf("envalloc_debug");
    /*Step 1: Get a new Env from env_free_list*/
	if(LIST_EMPTY(&env_free_list)) return -E_NO_FREE_ENV;
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
    ...
    /*Step 1: alloc a page. */
	if(page_alloc(&p)!=0){
		printf("page_alloc_error in load_icode\n");
		 return;
	}
	...
	if(r!=0){
		printf("load_icode error\n");
		return;
	}
	//e->env_status = ENV_RUNNABLE;
	//LIST_INSERT_HEAD(&env_sched_list[0],e,env_sched_link);
	//printf("%d load_icode_fin\n",e->env_id);
...
env_run(struct Env *e)
{
    ...
    *  switch the context and save the registers. You can imitate env_destroy() 's behaviors.*/
	if(curenv){
		...
		bcopy((void*)TIMESTACK - sizeof(struct Trapframe),&(curenv->env_tf),sizeof(struct Trapframe));
