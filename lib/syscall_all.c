...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
	...
	int ret;
	ret = 0;
	if(((perm&PTE_COW) == PTE_COW)||va>=UTOP||((perm&PTE_V)!=PTE_V))
		return -E_INVAL;
				...
				u_int perm)
{
    ...
    //your code here
	if(dstva>=UTOP||srcva>=UTOP||((perm&PTE_V)!=PTE_V))
		return -E_INVAL;
	...
	//if(ppte!=NULL&&((*ppte)&PTE_R==0)&&(perm&PTE_R!=0)) return -E_INVAL;
					 ...
					 u_int perm)
{
	...
	if(e->env_ipc_recving==1){
		...
		//printf("sendhere");
		if (srcva != 0) {
			if ((r=sys_mem_map(sysno,curenv->env_id,srcva,envid,e->env_ipc_dstva,perm))!=0) return r;
			e->env_ipc_perm = perm;
		}
		return 0;
	}else{
...
//	if((dev>=0x10000000&&dev+len>=0x10000000&&dev<0x10000020&&dev+len<0x10000020)||(dev>=0x13000000&&dev+len>=0x13000000&&dev<0x13004200&&dev+len<0x13004200)||(dev>=0x15000000&&dev+len>=0x15000000&&dev<0x15000200&&dev+len<0x15000200)){
//		bcopy((void *)va, (void *)(0xA0000000 + dev), len);
//		return 0;
//	}
//	return -E_INVAL;
