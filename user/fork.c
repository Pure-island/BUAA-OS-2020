...
pgfault(u_int va)
{
	...
	if ((perm & PTE_COW) != PTE_COW) {
		user_panic("PTE_COW_error\n");
	}
	else{
		tmp = UTOP-BY2PG*2;
		syscall_mem_alloc(0, tmp, perm&(~PTE_COW)|PTE_R);
		user_bcopy((void *)va, (void *)tmp, BY2PG);
		syscall_mem_map(0, tmp, 0, va, perm & (~PTE_COW)|PTE_R);
...
duppage(u_int envid, u_int pn)
{
	...
	//	}
	//}
	//syscall_mem_map(0, pn*BY2PG, envid, pn * BY2PG, perm);
...
fork(void)
{
	...
	for (i = 0; i < USTACKTOP; i += BY2PG) {
		if((((*vpd)[PDX(i)]&PTE_V)!=0)&&(((*vpt)[VPN(i)]&PTE_V)!=0))
			duppage(newenvid,VPN(i));
	//	if ((*vpd)[PDX(i)]&PTE_V!=0) {
	//		for( j = 0; j < PDMAP && i + j < USTACKTOP; j += BY2PG){
	//			if((*vpt)[VPN(i + j)]&PTE_V!=0){
